...
// light attack is used, depending on the opponent's attack.
{
	...
	User_Move3->move_type = 1;
	Opponent_Move3->computer_move_type = 3;

	user_damage = do_move(User_Move3->move_type, Opponent_Move3->computer_move_type, User);

	cout << "Test 3: Light attack vs Defend: damage expected to be between 9 and 29 (inclusive)" << endl;
	cout << "Damage inflicted: " << user_damage << endl;
	if (user_damage >= 9 && user_damage <= 29)
	{
